...
It would later occur that due to time constraints and absent team members I would develop large parts of the `Node Types`, `Node Execution`, `Unit Tests` and `Help Tool` workpackages myself quickly so to be able to focus on `Game Creation` tasks which were dragging behind.
...
For programming-related work carried out prior to the start of formal time logging, I utilize a proven Git-based time estimation algorithm, as detailed in @jonathan_est_tracked_efforts. To validate the reliability of this method, I compare its output against the formally logged time data recorded from July 2nd onward. This cross-comparison serves as a calibration step to assess the consistency and accuracy of the algorithm.

==== Estimation of Untracked Efforts

To estimate untracked efforts — such as work conducted in Typst, Figma, or via Discord — I will draw comparisons between the resulting output of these tasks and similar activities for which the time expenditure is known. This is possible because most tasks completed in the planning and conceptualization phases have equivalents in later stages. Concept drawings can be compared to assets I created and the creation of planning and presentation-related docuemnts to documents written during the documentation phase.

==== Estimation of Tracked Efforts<jonathan_est_tracked_efforts>

The algorithm used to estimate version controlled effort was first implemented by Kimmo Brunfeldt #footnote([https://github.com/kimmobrunfeldt]) under the name `git-hours` #footnote([https://github.com/kimmobrunfeldt/git-hours]). As taken from the official repository the algorithm works as follows @githubGitHubKimmobrunfeldtgithours:
