...
// Jonathan hat den absatz der hier vorher stand zur target audience nach oben geschoben (zum background). ich würde hier nur kurz erwähnen, dass unser didactisches konzept natürlich auf die Zielgruppe abgestimmt ist. ich weiß nicht ob wir hier schon das gesamte konzept "spoilern" wollen.
...
// ich würde nicht sagen, dass wir typescript nutzen, weil react flow in typescript geschrieben ist. das ist ja nicht wirklich ein argument. ich würde eher sagen, dass wir typescript nutzen, weil es uns hilft, fehler frühzeitig zu erkennen, die wartbarkeit und nutzbarkeit des codes zu verbessern
Since React Flow is written in TypeScript, we adopt TypeScript as our programming language of choice. TypeScript is a superset of JavaScript that adds static typing, which helps catch errors early in the development process and improves code maintainability. This is particularly beneficial for a project like ours, where we are building a complex system with many interconnected components. 
// das liest sich so als würde man sonst nicht tsx/jsx verwenden oder als wäre das irgendwie besonders. eigentlich verwendet man ja tsx/jsx IMMER im react context
To expand on easy development, we use TypeScript XML (TSX), a TypeScript syntax extension, to write our React components. TSX allows us to write HTML-like syntax within our TypeScript code, making it easier to create and manage the user interface. This is especially useful for building the node editor, where we need to create a variety of custom nodes with different properties and behaviors.
...
// ja dings ich schreib hier was, genrell zu markdown und warum wir das nutzen
To facilitate the integration of our documentation and interactive examples, we utilize MDX #footnote([https://mdxjs.com/]), which allows us to embed React (and by extension, TypeScript) components directly within Markdown files. This capability enhances the expressiveness and interactivity of our documentation, enabling us to illustrate concepts with live, executable components rather than static code snippets or textual descriptions. MDX thereby serves as a powerful medium for both technical explanation and dynamic visualization, bridging the gap between written content and functional interface elements in a seamless and developer-friendly manner.
