...
#col[
  ...
  React @React is a modern, free and open source JavaScript library for building user interfaces through a declerative, component-based architecture. Its efficient update model via a virutal DOM makes it ideal for developing interactive web applications. In our project, React serves as the foundation for our user interface, allowing us to create a dynamic and responsive experience for users. To avoid the overhead of implementing a complex node system from scratch, which would have been impossible within the set time frame of the project, we chose to use React Flow @ReactFlow2025 to create the node editor. React Flow is a library specifically designed for creating node and graph-based interfaces in React applications. It provides a set of essential features such as general node management, the canvas for the nodes and the ability to create fully custom sets of nodes. This allowed us to create the sophisticated node editor needed for our project. As our programming language of choice, we decided to use TypeScript @Typescript — a statically typed superset of JavaScript. TypeScript allows us to catch type-related errors early in the development process and contributes to clearer, more maintainable code. These benefits are particularly important in a project like ours, which involves a complex system with many interconnected components and evolving interfaces. Since we are working with React and TypeScript, we naturally use TSX — the standard syntax extension for TypeScript that allows us to write HTML-like markup within TypeScript code. This makes building and maintaining the user interface more intuitive, especially in areas like the node editor, where we create various custom components with specific properties and behaviors.


  == Evaluation of Game Engines
  Following our decisions to implement the game in 2D and to deploy it as a web-based application (see @2d_vs_3d, @web_vs_native), the next major consideration was selecting a suitable game engine. The engine would need to support our goals of educational clarity, web accessibility, and smooth integration with our React-based node editor. We evaluated several options, including Kaplay.js @KAPLAYjsHTML5Game and Unity @UnityRealTimeDevelopment.

  While Unity is a powerful and well-established engine it was not an ideal fit for our web-first, 2D educational tool. Integrating our web-based node editor — built using React and React Flow — into Unity would have required building a complex bridge between two fundamentally different runtimes, introducing technical overhead that would slow down development and hinder tight interaction between the editor and game simulation. Creating a seamless and responsive feedback loop between the node interface and game environment was central to our learning goals, and this level of interactivity is much more straightforward to achieve within a single web application.

  Kaplay.js, by contrast, was the most natural fit for our project. As a lightweight 2D web game engine, Kaplay aligns perfectly with our technical and pedagogical requirements. It provides all the essential features out of the box — such as scene management, physics, and collision detection — allowing us to focus on educational content rather than engine plumbing. The engine's 2D focus simplified the implementation of animation principles like translation and rotation, which are easier to grasp in two dimensions and more aligned with the cognitive load considerations that guided our overall design decisions @swellerCognitiveLoadProblem1988.

  Furthermore, because Kaplay is TypeScript-based, it integrates smoothly into the rest of our web tech stack. We could co-develop the game and node system in a unified codebase, enabling real-time feedback between user-generated programs and the visual simulation. This tight integration was critical to maintaining the immediacy and clarity necessary for effective learning.

  By choosing Kaplay, we stayed consistent with our project's core goals: simplicity, accessibility, and a focus on learning outcomes over technical flashiness. The engine allowed us to create a performant, browser-native, and easily maintainable learning tool — one that supports experimentation and understanding without the distractions or complications of a 3D or native application stack.

  == State Management with Zustand
  For global state management within our React application, we opted to use Zustand @PmndrsZustand2025 instead of React's built-in Context API. While React Context is a powerful tool for sharing data across a component tree, it is primarily designed for static or relatively infrequent updates within the React render cycle. In contrast, our application demands a more flexible architecture: the global state must be accessible and modifiable not only from React components, but also from external modules such as the game engine and custom event systems. Zustand excels in this scenario by allowing state to be accessed outside the component tree, without requiring a surrounding provider or deep prop-drilling. It also minimizes re-renders by enabling selective subscriptions to state slices, which helps maintain performance during rapid state updates. This design aligns well with our architectural needs and helps ensure both performance and maintainability throughout the project.
  ...
  Both stores include functions to save all of their internal data. These functions are called frequently as described in @loading_level, ensuring the user's progress is not lost.
  ...
  The `NodeStore` has a custom `AppNode` class which holds among other things, seperate maps for all of the connected input and output handles and the compute function. Each time a new node is added to the node editor the `NodeStore` adds a new `AppNode` object to the unsorted node map which it maintains. The `AppNode` receives the compute function directly from the react component defining each node.

  Each time a node is added or a connection is changed the unsorted map is ordered through a topological sorting algorithm based on depth-first search and saved seperately. The sorting algorithm marks the current node and then recursively goes through all of the nodes that a are conneted to the nodes output. Once it reaches a node that has no other nodes connect to its output it adds it to the sorted list and permanently marks the node. If the algorithm reaches a node with a non-permanent mark it means that a cyclic connection exists in the graph and an error message will be triggered.
  ...
  Initially, we explored an alternative method inspired by React Flow's suggested approach: building a reactive node graph using React's state system. In this model, each node listens to state changes in its inputs and re-computes itself accordingly. This effectively created a reactive node graph. Although it was possible in principle to implement all required systems (e.g. cyclic detection), doing so would have introduced significant complexity and performance issues as well as decoupled node execution from the gameloop. As a result, we opted for a more deterministic and scalable solution based on topological sorting and explicit graph execution, which gives us full control over cyclic validation, computation order, and performance characteristics.
