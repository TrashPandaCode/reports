...
The real dataset consited of manually measured RT60 values from various office environments, captured using our acoustic measurement setup/*add reference*/. The setup included a microphone, an omnidirectional speaker, and a laptop running our custom measurement software/*reference leos python script*/. Despite comprising only 300 datapoints across approximately 70 rooms, this dataset provided crucial ground truth validation and represented authentic acoustic-visual relationships that exist in practical environments.

The limited size of the real dataset was primarely constrained by practical considerations, including the substantial time investment required for proper manual RT60 measurement procedures and logistical limitations in accessing sufficient diverse office spaces within the university. Manual RT60 measurements, while providing high accuracy, requires controlled conditions and our specialized equipment, making large-scale data collection prohibitively resource-intensive./*include time reference if deemed necessary*/

=== Synthetic Data Generation 
The synthetic dataset was created using a custom sophisticated pipeline combining Blender for 3D rendering and Treble for acoustic simulation. The Blender environment enabled systematic randomization of key visual parameters including exterior environments visible through windows, ceiling configurations, chair arrangements, table placements, wall materials and camera parameters. This parametric approach ensured comprehensive coverage of potential office room configurations while maintaining computational efficiency.
