...
#col[
  ...
  *Considerations:* The user problem is both relevant and serious, especially in educational contexts where abstract animation concepts are often hard to grasp. Many students struggle with conventional programming tools due to their complexity and steep learning curves. This project addresses that gap by offering an interactive, visual alternative designed to reduce cognitive load and improve comprehension. Solving this problem is essential to support more inclusive, effective learning experiences in digital media and animation education.
  ...
  The project fits within the broader educational technology and game development environment, targeting users who seek to learn computer animation through interactive tools. The work context includes the following adjacent systems:

  #todo("polish the wording of the following Stichpunkte")

  - *Frameworks:* Our Framework is #link("https://react.dev/", [_React_]) since we just finished a course teaching #link("https://react.dev/", [_React_]) last semester. Also #link("https://react.dev/", [_React_]) is probably the most established Web Framework and we found Reactflow which works with #link("https://react.dev/", [_React_]).
  - *UI:* Our User Interface relies on  #link("https://reactflow.dev/", [_Reactflow_]) for the node editor. This includes the Canvas on which the editor is drawn and much of the underlying events we build on that are assosiated with nodes and their canvas (such as dragging or selecting and many others).
  #colbreak()
  - *Statemanagment:* We use #link("https://zustand-demo.pmnd.rs/", [_Zustand_]) to manage State in our Applications. #link("https://zustand-demo.pmnd.rs/", [_Zustand_]) provides the ability to create so called stores in which the state of certain components can be stored. This is useful because the React native ways such as useState and useContext hooks get messy very quickly. Having the State in these concentrated stores also makes it way easier to persist them in the local storage of the browser.
  - *Game Engines:* We are using #link("https://kaplayjs.com/", [_Kaplay_]) as our game engine since it ticks all the boxes. It is a game engine for the web, it works with #link("https://react.dev/", [_React_]) and is specialized for two dimensional games.

  These Technologies help us eleviate some of the developement work and therefore focus on implementing the nodes and game as we see fit.
  
  // Understanding these adjacent systems helps clarify how our game will be used, how it complements existing resources, and what interfaces or data exchanges are necessary. Defining this context ensures the product aligns with user workflows, educational goals, and technical ecosystems, preventing isolated development and enhancing overall usability and adoption.
]
#figure(caption: "The technologies we rely on", image("../images/WEB - Context of the Work.png"))
...
#table(
  ...
  [*University Students*], [Key Users], [\~85%], [Students are the primary audience and main users. The product’s success is defined by its ability to engage them and support their learning. Their feedback and experience are essential to shaping usability and feature development.],
  [*Instructors / Teaching Staff*], [Secondary Users], [\~10%], [While not the main users, instructors influence adoption in academic settings. Their approval is important for integration into courses, but they use the product less frequently. Their requirements are important but take second place to student needs in case of conflict.],
  [*Casual Viewers / Passers-by*], [Unimportant Users], [\~5%], [These users may briefly interact with the system during demonstrations or events but are not part of the intended user base. Their input does not guide product development or requirements.],
)
